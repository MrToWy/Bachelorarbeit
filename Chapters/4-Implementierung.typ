...
Ein erster Ansatz zur Erstellung einer Methode für das Erstellen und Bearbeiten eines Modules ist es, das erhaltene JSON direkt an den Prisma-Client zu übergeben und zu hoffen, dass der Prisma-Client das JSON korrekt verarbeitet.
...
#codeFigure("Containerfile für das Frontende", <frontendDockerfile>, "FrontendDockerfile")


Nachdem nun alle Containerfile bereitstehen, können die Images erstellt werden. Der Befehl `podman build -t studymodules-documentation .` erstellt beispielsweise das Image für die Dokumentation. Damit aus den Images nun Container generiert werden können, wurde eine compose-Datei erstellt (@compose). In dieser ist beschrieben, wie die Container konfiguriert sein sollen. Alle Container werden zur besseren Übersicht in einer Gruppe names "studymodules_project" gruppiert. Der Container latex-poll-script ist am einfachsten zu erklären. Bei diesem ist lediglich das Image angegeben, auf dem er basiert. Da das latex-poll-script nur zwischen Backend und latex-api vermittelt, sind keine weiteren Konfigurationen notwendig.
