...
Im gezeigten Codebeispiel werden zunächst alle verfügbaren Felder eines Moduls ermittelt. Hierzu wird die Methode Object.keys @ObjectKeysJavaScript2023 verwendet, welche ein Array aller Felder des Moduls zurückgibt. Die erhaltenen Felder werden im Anschluss in die Kategorien "Arrays", "Übersetzungen" und "Primitive Felder" aufgeteilt (@compareFields).
...
Ein UI-Framework kann bei der Implementierung des Frontends unterstützen. Vom Framework angebotene vorgefertigte Komponenten müssen nicht selbst implementiert werden. Nach dem Don't-Repeat-Yourself-Prinzip wird dadurch der Entwicklungsaufwand reduziert und die Codequalität verbessert. @clean_code_2012_robert_martin Allerdings muss die Nutzung eines Frameworks sorgfältig abgewogen werden. Jede Einführung zusätzlicher Abhängigkeiten birgt gewisse Risiken. Das Framework könnte eine Sicherheitslücke beinhalten @NpmThreatsMitigations oder könnte zu anderen (zukünftigen) Abhängigkeiten inkompatibel sein. Diese Risiken können durch eine vorherige Untersuchung der Frameworks verringert werden. Wenn ein Framework open source ist und eine große Anzahl an Unterstützern hat, ist es theoretisch möglich, dass Sicherheitslücken zügig gefunden und behoben werden @OpenSourceSicherheitVerstaendlichErklaert. Da dieses Projekt keine geheimen oder personenbezogenen Daten verarbeitet, wird das geringe Risiko, das mit der Verwendung eines Frameworks einhergeht, eingegangen - zugunsten einer angenehmeren Entwicklung und eines schöneren Ergebnisses.
...
Außerdem werden beim Wechseln der Anzeigesprache die angezeigten Daten in jeder Maske neu geladen. Hierzu wird beim Öffnen der Maske wie in @uebersetzbarkeit beschrieben ein BehaviourObject genutzt. Auch dieses sollte beim Schließen der Maske deabonniert werden. Andernfalls gibt es die Gefahr von Memory Leaks - solange Events abonniert sind, kann die Maske nicht aus dem Arbeitsspeicher entladen werden. @AngularComponentLifecycle

Aus diesen beiden Gründen muss in einigen Masken das Interface OnDestroy @AngularComponentLifecycle implementiert werden. Dieses wird beim Schließen des Components aufgerufen. Der darin enthaltene Code (@onDestroy) ist dann recht simpel - das Intervall wird gestoppt und die Subscription auf das languageSubject wird deabonniert.
